   10 MODE 7
   20 BL$=CHR$(255) :DIM EPTY(0) :REM BLOCK
   30 NBL$=" "      :REM NO BLOCK
  ...
  150 FOR SX=0 TO 10 : FOR SY=0 TO 17 : SCREEN$(SX,SY)=NBL$ : NEXT SY : NEXT SX :REM FOR WRITING BLANKS TO ALL SLOTS
 ...
 2382 SX=0
 2383 SY=0
 2384 SY=SY+1 : IF SY>3 THEN SY=0 : SX=SX+1 : IF SX>3 THEN GOTO 2410
 2390 IF X+1 > 10 THEN ENDPROC :REM HARD CHECK ON WALL
 2400 IF SETHB$(R,SX,SY) = BL$ : IF SETHB$(R,SX,SY) = SCREEN$(SX+X+1,SY+Y) THEN DP=TRUE
 2405 GOTO 2384
 2410 IF DP=FALSE THEN PRINTTAB(X+14,Y+3)NSET$(R) : X=X+1 : PROC_PRINTSHAPE
 2420 ENDPROC
 ...
 2506 SX=0
 2507 SY=0 : GOTO 2510
 2509 SY=SY+1 : IF SY>3 THEN SY=0 : SX=SX+1 : IF SX>3 THEN GOTO 2518
 2510 IF SETHB$(R,SX,SY) = BL$ : IF SETHB$(R,SX,SY) = SCREEN$(SX+X,SY+Y+1) THEN DP=TRUE
 2511 GOTO 2509
 2518 REM PROMT
 2520 IF DP=TRUE THEN FOR SX=0 TO 3 : FOR SY=0 TO 3 : SCREEN$(SX+X,SY+Y)=SCREEN$(SX+X,SY+Y)+SETHB$(R,SX,SY) : NEXT SY : NEXT SX : PROC_CHECKREMOVELINE : PROC_NEWBLOCK
