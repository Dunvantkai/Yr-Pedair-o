   ...
   90 X=0 : Y=0 : R=0 :REM X Y AND ROTAION
   95 DIM SCREEN$(14,21) : DIM SETHB$(3,3,3)  :REM SCREAN SIZE AND SELECTED HITBOX FOR THE SPITES
  100 REM COLOUR
 ...
 1780 AMSER%=0 :REM WHILE TRUE:REM REPEAT
 ...
 1830 IF K%=112 OR K%=80 THEN :REM P FOR DROP
 1840 IF K%=108 OR K%=76 THEN PROC_DOWN :REM PROC_RESET :REM L FOR EXIT
 1850 REM AMSER%=AMSER%+1
 1860 REM IF AMSER%=1000 THEN PROC_DOWN
 1910 GOTO 1790 :REM UNTIL FALSE FIX LATTER CANT BE LEFT  :REM 790
 1920 ENDPROC
 1930 DEF PROC_Print
 1940 REM PRINT SBL$ :REM CLS
 1950 REM PRINTTAB(X+14,Y+3)SET$(R)
 1955 FOR SX=0 TO 11 : FOR SY=0 TO 18 : PRINTTAB(SX+14,SY+3) SCREEN$(SX,SY) : NEXT SY : NEXT SX
 ...
 2350 FOR SX=0 TO 3 : FOR SY=0 TO 3 : IF SETHB$(R,SX,SY) = SCREEN$(SX+X-1,SY+Y) THEN XN=1 : NEXT SY : NEXT SX
 2360 IF XN=0 THEN X=X-1 : PRINTTAB(X+14,Y+3)NSET$(R) : PROC_PRINTSHAPE
 2370 ENDPROC
 2380 DEF PROC_RIGHT
 2390 IF X+1 > 11 THEN ENDPROC :REM HARD CHECK ON WALL
 2400 FOR SX=0 TO 3 : FOR SY=0 TO 3 : IF SETHB$(R,SX,SY) = SCREEN$(SX+X+1,SY+Y) THEN XP=1 : NEXT SY : NEXT SX
 2410 IF XP=0 THEN X=X+1 : PRINTTAB(X+14,Y+3)NSET$(R) : PROC_PRINTSHAPE
 2420 ENDPROC
 2430 DEF PROC_ROTATE
 2440 ROT=R
 2450 ROT=ROT+1
 2460 IF ROT>3 THEN ROT=0
 2470 FOR SX=0 TO 3 : FOR SY=0 TO 3 : IF SETHB$(ROT,SX,SY) = SCREEN$(SX+X,SY+Y) THEN ENDPROC ELSE NEXT SY : NEXT SX
 2480 R=R+1 : PRINTTAB(X+14,Y+3)NSET$(R) : PRINTTAB(X,Y)"HEY" : PROC_PRINTSHAPE
 2490 ENDPROC
 2500 DEF PROC_DOWN
 2510 REM FOR SX=0 TO 3 : FOR SY=0 TO 3 : IF SETHB$(ROT,SX,SY) = SCREEN$(SX+X,SY+Y-1) THEN ;
 2520 FOR SX=0 TO 3 : FOR SY=0 TO 3 : SCREEN$(SX+X,SY+Y)=SCREEN$(SX+X,SY+Y)+SETHB$(R,SX,SY) : NEXT SY : NEXT SX : ENDPROC
 2530 REM ELSE NEXT SY : NEXT SX
 2540 ENDPROC
