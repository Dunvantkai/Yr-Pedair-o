   ...
   95 DIM SCREEN$(13,20) : DIM SETHB$(3,3,3)  :REM SCREAN SIZE AND SELECTED HITBOX FOR THE SPITES
  100 REM COLOUR
 ...
 1800 IF X>10 THEN X=10
 1810 IF X<0 THEN X=0
 1820 IF Y>17 THEN Y=17
 1830 IF Y<0 THEN Y=0
 1840 IF K%=87 OR K%=119 THEN PRINTTAB(X+14,Y+3)NSET$(R) : Y=Y-1 :PROC_Print :REM W
 1850 IF K%=83 OR K%=115 THEN PRINTTAB(X+14,Y+3)NSET$(R) : Y=Y+1 :PROC_Print :REM S
 1860 IF K%=68 OR K%=100 THEN PRINTTAB(X+14,Y+3)NSET$(R) : X=X+1 :PROC_Print :REM D
 1870 IF K%=65 OR K%=97  THEN PRINTTAB(X+14,Y+3)NSET$(R) : X=X-1 :PROC_Print :REM A
 1880 IF K%=81 OR K%=113 THEN PRINTTAB(X+14,Y+3)NSET$(R) : R=R+1 :IF R>3 THEN R=0 :REM ROTATES PEASE AND MAKE SURE IT STAYS IN BOUNDS
 ...
 1950 PRINTTAB(X+14,Y+3)SET$(R)
 1955 FOR SX=0 TO 13 : FOR SY=0 TO 20 : PRINTTAB(SX+14,SY+3) SCREEN$(SX,SY) : NEXT SY : NEXT SX
 1970 PROC_SCREEN
 ...
 2160 PRINTTAB(13,0)L$Y$"PEDAIR--O" : PRINTTAB(13,1)L$M$"PEDAIR--O" : PRINT X : PRINT Y
 2170 ENDPROC
