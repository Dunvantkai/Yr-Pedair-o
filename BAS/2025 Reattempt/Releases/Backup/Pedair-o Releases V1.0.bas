...
X=0 : Y=0 : R=0 : COL=FALSE : THISREMOVEDLINE%=0 : TMP$=0 :REM PLAYER X,Y,R / COLLISION / CURSOR LINE REMOVE  
...
IF K%=69 OR K%=101 THEN PROC_COLOUR : GOTO """COLOUR-LOOP"""
...
ENDPROC
DEF PROC_LEFT
FOR SX=0 TO 3:FOR SY=0 TO 3:TMP$=SETHB$(R,SX,SY):IF TMP$=BL$:IF TMP$=SCREEN$(SX+X-1,SY+Y) THEN COL=TRUE :REM Collision Check
NEXT SY : NEXT SX
...
ENDPROC
DEF PROC_DOWN
FOR SX=0 TO 3:FOR SY=0 TO 3:TMP$=SETHB$(R,SX,SY):IF TMP$=BL$:IF TMP$=SCREEN$(SX+X,SY+Y+1) THEN COL=TRUE :REM Collision Check
NEXT SY : NEXT SX
IF COL=FALSE THEN PRINTTAB(X+14,Y+3)NSET$(R) : Y=Y+1 : PRINTTAB(X+14,Y+3)SET$(R) :REM Collision Failed
IF COL=TRUE THEN COL=FALSE : SCORE%=SCORE%+5 : FOR SX=0 TO 3 : FOR SY=0 TO 3 : SCREEN$(SX+X,SY+Y)=SCREEN$(SX+X,SY+Y)+SETHB$(R,SX,SY) : NEXT SY : NEXT SX : PROC_NEXTBLOCK :REM Collision Successful
ENDPROC
DEF PROC_UI
CLS
...
FOR YA=0 TO 17 : FOR XA=0 TO 10 : SCREEN$(XA,YA)=SCREEN$(14,21) : NEXT XA : NEXT YA
ENDPROC
DEF PROC_NEXTBLOCK
